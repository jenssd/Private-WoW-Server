...
-- Curated starter sets for a melee/enhancement shaman.
-- The system is deliberately data-driven so additional classes, specs and
-- level brackets can be added without rebuilding the UI.
...
subtitle:SetText("Schamane - Verstaerkung / Melee")
...
rangeLabel:SetPoint("TOPLEFT", frame, "TOPLEFT", 18, -79)
rangeLabel:SetText("Levelbereich")

local rangeButton = CreateButton(frame, "", 130, 24, 18, -99, function()
    local ranges = PrivateWoWAdminGearSets.catalog.shaman_enhancement.ranges
...
modeLabel:SetPoint("TOPLEFT", frame, "TOPLEFT", 175, -79)
modeLabel:SetText("Staerke")

local balancedButton = CreateButton(frame, "Ausgewogen", 105, 24, 175, -99, function()
    selectedMode = "balanced"
    PrivateWoWAdminGearSets.Refresh()
end)
local strongButton = CreateButton(frame, "Stark", 85, 24, 285, -99, function()
    selectedMode = "strong"
    PrivateWoWAdminGearSets.Refresh()
end)
local heirloomButton = CreateButton(frame, "Erbstuecke", 105, 24, 375, -99, function()
    selectedMode = "heirloom"
    PrivateWoWAdminGearSets.Refresh()
end)

local description = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
description:SetPoint("TOPLEFT", frame, "TOPLEFT", 18, -137)
...
for index = 1, 8 do
    ...
    row:SetPoint("TOPLEFT", frame, "TOPLEFT", 24, -170 - ((index - 1) * 25))
    row:SetWidth(520)
...
local giveButton = CreateButton(frame, "Komplettes Set geben", 180, 28, 18, -382, function()
    local catalog = PrivateWoWAdminGearSets.catalog.shaman_enhancement
    local range = catalog.ranges[selectedRangeIndex]
...
CreateButton(frame, "Itemdatenbank", 130, 28, 210, -382, function()
...
local function SelectRangeForCurrentLevel()
    local level = UnitLevel("player") or 1
    selectedRangeIndex = level >= 10 and 2 or 1
end
